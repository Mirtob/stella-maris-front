// Genera un PDF con el Informe Final + Índice + Plan de Riesgos + Informe Presupuestario.
// Uso: node scripts/build-informe-pdf.mjs
import { readFileSync, writeFileSync } from 'node:fs';
import { chromium } from 'playwright';

const OUT = 'C:/Users/gusta/Downloads/Informe-Final-StellaMaris.pdf';

// title = título de portada de sección; file = ruta relativa al repo.
const FILES = [
  { title: 'Informe Final', file: 'docs/INFORME-FINAL.md' },
  { title: 'Índice de Documentación', file: 'docs/README.md' },
  { title: 'Plan de Riesgos', file: 'docs/entrega/PLAN-DE-RIESGOS.md' },
  { title: 'Informe Presupuestario', file: 'docs/entrega/INFORME-PRESUPUESTARIO.md' },
];
...
const inline = (s) =>
  esc(s)
    .replace(/^\[ \]\s/, '☐ ')
    .replace(/^\[[xX]\]\s/, '☑ ')
    // enlaces markdown → solo el texto (para impresión)
    .replace(/\[([^\]]+)\]\([^)]*\)/g, '$1')
...
<div class="cover">
  <div class="star">✶</div>
  <h1 class="cover-title">Informe Final</h1>
  <p class="cover-sub">Estado del proyecto · Documentación · Riesgos · Presupuesto</p>
  <p class="cover-org">Stella Maris · Cantoral Litúrgico Católico</p>
  <div class="cover-note">
    <p>Este documento reúne, para dirección y traspaso, el <strong>estado del proyecto</strong>,
    el <strong>índice de la documentación</strong>, el <strong>plan de riesgos</strong> detallado y el
    <strong>informe presupuestario</strong> (valor económico en UF y USD).</p>
    <p>Todos los documentos son <em>actualizables</em>; el estado vigente se mantiene en el
    Informe Final del repositorio (<code>docs/INFORME-FINAL.md</code>).</p>
...
const html = `<!doctype html><html lang="es"><head><meta charset="utf-8"><style>
  ...
  *{box-sizing:border-box}
  ...
  .cover-title{ font-size:36pt; margin:6px 0 4px; letter-spacing:-.01em; }
  .cover-sub{ font-size:13pt; color:var(--soft); margin:0; }
...
const tmp = 'scripts/.informe.tmp.html';
...
await page.pdf({
  ...
  footerTemplate: '<div style="width:100%;font-size:8pt;color:#9a8;text-align:center;font-family:Georgia,serif;">Stella Maris · Informe Final — <span class="pageNumber"></span>/<span class="totalPages"></span></div>',
});
await browser.close();
